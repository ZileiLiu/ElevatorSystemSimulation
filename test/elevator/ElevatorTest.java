...
public class ElevatorTest {
  ...
  /**
   * Set up the stuff before all the tests, so we can do a static test.
   */
  @BeforeClass
   ...
   * then reports STOPPED when it returns to the first floor.
  ...
  public void elevatorReportsStopped() {
    ...
    elevator.start();
    // wait for a job and then give up
    for (int i = 0; i < 5; i++) {
      elevator.step();
    }
  ...
  /**
   * Test that the elevator can be taken out of service when it is not on the  floor
   * and has no requests.
   */
  @Test
  public void elevatorOutOfService() {
    ...
    assertFalse(elevator.isTakingRequests());
    // it should now move to the first floor
    for (int i = 0; i < 3; i++) {
      elevator.step();
    }
    assertEquals(elevator.getCurrentFloor(), 0);
  ...
  /**
   * test that an elevator mid run reports it is not taking requests
   ...
   * the first floor with the door closed.
  ...
  public void stressTest() {
    ...
    for (int i = 0; i < 100000; i++) {
      ...
      // check that the elevator is on the first floor
      assertEquals(0, elevatorTenFloors10Capacity.getCurrentFloor());
